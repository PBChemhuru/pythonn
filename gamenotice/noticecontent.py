...
def exp():
    URL = "https://www.epicgames.com/store/en-US/browse?sortBy=releaseDate&sortDir=DESC&priceTier=tierDiscouted&count" \
          "=40&start=0 "
    page = requests.get(URL)
    soup = BeautifulSoup(page.content, "html.parser")
    result = soup.find(class_="css-cnqlhg")
    ...
    for game_element in game_elements:
        game_info = game_element.find("a", class_="css-1jx3eyg")
        links = game_element.findall("a")
        linka = []
        for link in links:
            # link_url = link["href"]
            linka.append(link.get('href'))
            # print(f"link here: {link_url}\n")
            print(linka)
